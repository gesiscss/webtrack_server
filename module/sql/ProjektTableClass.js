...
module.exports = class ProjektTableClass extends Core{
  ...
  createTable(){
    return db.promiseQuery("CREATE TABLE IF NOT EXISTS `"+this.table+"` ( `ID` INT(255) NOT NULL AUTO_INCREMENT, `NAME` VARCHAR(255) NOT NULL, `DESCRIPTION` TEXT NOT NULL, `ACTIVE` BOOLEAN NOT NULL DEFAULT FALSE, `ENTERID` BOOLEAN NOT NULL DEFAULT TRUE, `CHECK_CLIENTIDS` BOOLEAN NOT NULL DEFAULT FALSE, `SENDDATAAUTOMATICALLY` BOOLEAN NOT NULL DEFAULT TRUE, `PRIVATEBUTTON` BOOLEAN NOT NULL DEFAULT TRUE, `SHOWHISTORY` BOOLEAN NOT NULL DEFAULT TRUE, `EDITHISTORY` BOOLEAN NOT NULL DEFAULT FALSE, `SHOW_DOMAIN_HINT` BOOLEAN NOT NULL DEFAULT FALSE, `FORGOT_ID` BOOLEAN NOT NULL DEFAULT FALSE, `PRIVATETAB` BOOLEAN NOT NULL DEFAULT FALSE, `EXTENSIONSFILTER` VARCHAR(255) NOT NULL DEFAULT '{\"all\":false,\"js\":false,\"css\":false,\"img\":false,\"video\":false, \"music\":false}',  `CREATEDATE` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY(`ID`) ) ENGINE = InnoDB CHARSET=utf8mb4 COLLATE utf8mb4_german2_ci;");
  }
   ...
   */
  getAktiv(){
    return db.promiseQuery("SELECT `ID`, `NAME`, `DESCRIPTION` FROM `"+this.table+"` WHERE `ACTIVE` = 1");
  }
